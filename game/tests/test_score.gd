extends RefCounted
## Score.tally() - the five categories the end-of-run screen adds into one
...
func test_margin_and_standing_score_from_the_runs_own_totals() -> void:
	...
	r.finish_shift({"margin_banked": 4200, "quota": 3600, "standing_delta": 0,
		"customers_walked": 0, "combo_events": []})
...
func test_walkouts_dock_a_flat_amount_each_summed_across_the_whole_run() -> void:
	...
	r.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 2, "combo_events": []})
	r.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 1, "combo_events": []})
	var score := Score.tally(r)
...
func test_combo_points_reward_the_length_of_a_streak_not_just_the_sale_count() -> void:
	...
	## different totals, or calling it a "combo" would be a lie.
	var clean := _run(1)
	clean.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 0, "combo_events": [1, 2, 3], "combo_streak_end": 3})
	var clean_score := Score.tally(clean)
	h.eq("a clean streak of 3 scores 1+2+3 combo steps",
		clean_score["combo_points"], (1 + 2 + 3) * Score.POINTS_PER_COMBO_STEP)
	h.eq("its peak is remembered for display", clean_score["best_combo"], 3)

	var broken := _run(2)
	broken.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 2, "combo_events": [1, 1, 1], "combo_streak_end": 0})
	var broken_score := Score.tally(broken)
	h.eq("the same three sales, each starting the streak over, score far less",
		broken_score["combo_points"], 3 * Score.POINTS_PER_COMBO_STEP)
	h.check("strictly less than the unbroken streak despite equal sales",
		broken_score["combo_points"] < clean_score["combo_points"])

func test_a_streak_keeps_climbing_across_shifts_that_carried_it() -> void:
	## combo_events only ever holds the values THIS shift's closes landed on -
	## a streak that started in shift 1 and continued into shift 2 shows up as
	## [3] in shift 2's own events, not [1, 2, 3] repeated. Score still has to
	## credit the full climb once both shifts are summed.
	var r := _run()
	r.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 0, "combo_events": [1, 2], "combo_streak_end": 2})
	r.finish_shift({"margin_banked": 0, "quota": 100, "standing_delta": 0,
		"customers_walked": 0, "combo_events": [3], "combo_streak_end": 3})
	var score := Score.tally(r)
	h.eq("every step of the carried-forward climb scores",
		score["combo_points"], (1 + 2 + 3) * Score.POINTS_PER_COMBO_STEP)
	h.eq("and the peak reflects the shift that reached it", score["best_combo"], 3)

func test_the_total_is_exactly_the_sum_of_the_five_lines_shown() -> void:
	var r := _run()
	r.finish_shift({"margin_banked": 1000, "quota": 100, "standing_delta": -10,
		"customers_walked": 1, "combo_events": [1], "combo_streak_end": 0})
	var score := Score.tally(r)
	h.eq("the headline total is exactly what the five category lines add up to",
